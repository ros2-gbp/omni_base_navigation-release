...
def filters(context, launch_description):
    actions = []
    add_on_module = read_launch_argument('add_on_module', context)

    if add_on_module in ['horizon']:
        fllor_filter_node = 'floor_filter'

        floor_filter_config = get_pal_configuration(
            pkg='pcl_ros',
        ...
        actions.append(point_cloud_filters)

    else:
        raise ValueError(f"Unsupported add on module: {add_on_module}")
...
def declare_actions(
    ...
    floor_filters = OpaqueFunction(
        function=filters,
        args=[launch_description],
    )
    launch_description.add_action(floor_filters)
